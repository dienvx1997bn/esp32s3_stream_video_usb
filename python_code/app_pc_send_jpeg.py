...
def split_data_to_fit_size(data):
    ...
    # last data
    # print(f"append last data index {i}")
    if(remainder > 0):
        my_array = [i & 255, i >> 8, remainder, 0]
        while j < remainder:
            my_array.append(data[byte_added])
            byte_added += 1
            j+= 1
...
def test():
    # file_path = 'images/frame_2.jpg'
    file_path = 'esp32.jpeg'
    with open(file_path, 'rb') as file:
        image_bytes = file.read()
    
    data_to_send = split_data_to_fit_size(image_bytes)
    split_data = split_list_by_byte_size(data_to_send, 64)
    ...
    for filename in sorted_files:
        ...
        if filename.endswith(".jpg"):
            ...
            with open(file_path, 'rb') as file:
                ...
                byte_arrays.append(image_bytes)
